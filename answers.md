...
Head over to [this website](https://www.virtualbox.org/) and click on the blue button in the middle of the page that says Download VitualBox.

It redirected me to this [page](https://www.virtualbox.org/wiki/Downloads). Once I was here I clicked the OS X host and it began to download a DMG file on my machine. 
...
1. Double click on the brown box icon: VirtualBox installation window should open. 

2. Inside of Install Oracle VM VirtualBox, click continue and install. 

3. When the installation is completed successfully click close.

Once the VitualBox is installed its time to install vagrant.

I went to this [page](https://www.vagrantup.com/downloads.html) and clicked on the MacOs version.

Now to get Vagrant up and running I typed in these command lines into the terminal:
...
Once everything is set up on my machine I signed up for Datadog's 14-day free trial.

Once signed into Datadog, I needed to set up the Agent. First click on Integrations tab, then selected agent and then clicked on Ubuntu since that is the integration we will be using.

Use the command line given to install the Datadog Agent on the virtual machine.

### Collecting Metrics: ###

Tags are a way to distinguish between hosts. 

First I adapted the Agent configuration file. 

We can set up datadog tags by editing datadog.yaml. It can be found \etc\datadog-agent\datadog.yaml

To edit the file I needed to run the command: sudo vim datadog.yaml
...
Once this is done I needed to install the respective Datadog integration for that database.
...
I then ran $ sudo datadog-agent status to make sure that my output contained MySql. 
...
After that file is created and saved, need to create a yaml file with the same name as the python file inside the /etc/-agent/checks.d directory only this time it has to be inside of /etc/-agent/conf.d directory. I named mine hello.yaml.
...
On Datadog's website click on the integrations tab and select APIs. On that page, I was able to find the api_key and app_key that I needed to create the script. 
...
2. Customize the title and description.
...
5. For the second definition, I needed to write an anomaly algorithm. I found the initial documentation reference [here](https://docs.datadoghq.com/graphing/miscellaneous/functions/#algorithms). It provided an easy to understand overview. 
...
In the datadog homepage, I clicked on the dashboard tab and select dashboard list. I then clicked on the timeboard most recently made. Then all I needed to do was hover the cursor over the graph and selected the 5 minute time frame. 
...
Then I set up the alerts. Depending on which monitor is triggered the message needs to be catered to it. {{#is_alert}} for example will let me know if the data output is over 800. I included all three alerts, the warning and the scenario in which there is no data.
...
The following images depict how I set up the downtime monitor for 7 pm to 9 am EST(11pm to 1pm UTC) as well as weekends.
...
First, we need to install [flask](http://flask.pocoo.org/docs/0.12/installation/#installation) and [ddtrace](http://pypi.datadoghq.com/trace/docs/#) onto our VM. 

Following Flask’s installation, we can use the Flask app provided by the Datadog challenge. I created a python file and used ddtrace to start the python script and then hit the endpoints (ex. 0.0.0.0:5050/api/trace).
