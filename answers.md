...
I first created a working example "reference" Timeboard, via the GUI and used the API to issue a query to show a list of all my timeboards with  their ID numbers.  I then used the *timeboard_id* of the example timeboard to issue an API pull to download  the json definition of that timeboard, written to a file (*testtimeboard*).  I utilized *dogshell* to issue these API queries.  In this endeavor *dogshell* was my friend.  Dogshell is handy tool to have 
...
While my analysis validated that their perception was accurate, the process of creating that analysis was slow and tedious.  Using Datadog  the whole pipeline and the systems used to enable it could have been instrumented.  In this casee the whole analyis could have been automated.  A timeboard of disk failures graphed against a calculated predicted failure rates automatically based on metrics of # of disks in operation would have exposed the issue right away.  And additional instrumentation could have triggered alerts that theshholds of operational metrics (render rate, render latency, a disk failure threshold per NAS system) were exceeded.   Reviewing Datadog documentation on using Datadog to monitor **SNMP** enabled devices this appears to quite feasible for "black box" appliance systems like enterprise NAS solutions.  
