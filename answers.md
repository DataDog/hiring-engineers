...
For my agent, I opted to configure the recommended Linux VM using Vagrant.  The machine is running Ubuntu-18.04 and is hosted on my desktop.  

To get my environment up and running I performed the following:

    $ vagrant init hashicorp/bionic64
    $ vagrant up
    $ vagrant ssh
    vagrant@vagrant$ DD_AGENT_MAJOR_VERSION=7 DD_API_KEY=<API_KEY> DD_SITE="datadoghq.com" \ 
...
To add tags, I modified `/etc/datadog-agent/datadog.yaml` to include tags for `envirionment` and `project` by adding the following in the `@params tags` section:
...
Once completed, I restarted my agent with `sudo service datadog-agent restart`.  I was then able to view the newly available metrics found under Metrics > Explorer as seen below:
...
I started by creating `/etc/datadog-agent/checks.d/custom_my_metric.py` with the following:
...
Next, I created `/etc/datadog-agent/conf.d/custom_my_metric.yaml` with the following:
...
I modified `/etc/datadog-agent/conf.d/custom_my_metric.yaml` to change the default value of `min_collection_interval` from 15 seconds to 45 with the following:
...
Yes, I opted to set the `min_collection_interval` in the configuration file instead of modifying the python check file.
...
> I initially searched for a Timeboard API and noticed in the documentation found [here](https://docs.datadoghq.com/dashboards/guide/timeboard-api-doc/?tab=python) that this endpoint was outdated and followed the instructions to instead use the Dashboard endpoint as part of the Datadog API found [here](https://docs.datadoghq.com/api/latest/dashboards/)

I first went about acquiring my API Key, and my App Key found under Integrations > APIs so that I could define them under the `options` object in my script. I then created [`\code\create_dashboard.py`](code/dashboard.py).

### **Show your custom metric scoped over your host.**

I learned that all dashboards are made up of [widgets](https://docs.datadoghq.com/dashboards/widgets/), which are individual charts that can present metrics with a variety of visualizations.

For the first widget scoped over `my_metric`, I chose a `timeseries` widget as the data provided by the custom check sends a single data-point every 45 seconds.  The schema for this widget is as follows:
...
> I scoped the query to `{host:vagrant}` since I had experimented in previous sections with configuring the agent on my local desktop.  Although not strictly necessary, I did this for all subsequent sections for readability and consistency.  
...
After running the script, the [final dashboard](https://p.datadoghq.com/sb/7my7we8zuethfnx9-db6234b107a2374c39d85644a3344e69) I produced looks like this:

![Final Dashboard](screenshots/dashboard.PNG "Final Dashboard")

> In addition to my script `/hiring-engineers/code/dashboard.py`, there is a script `/hiring-engineers/code/clear_dashboards.py`, which I used as a convenience when testing a variety of dashboard widgets. 
...
> Since the `query_value` widget is set to a 1-hour roll-up, no data is present since there isn't a 1-hour roll-up window present for a 5-minute window.
...
I opted to take a snapshot of the My Metric widget showing `my_metric` graphed over a time-series since I could not take a snapshot of the full dashboard, and it shared a more interesting set of data than the other widgets. 
...
The anomaly graph in my dashboard shows actual values against a band of values considered “normal” relative to a standard deviation of 3.  Anything outside of that band would be considered an anomaly. I am using the basic algorithm, which performs a simple lagging rolling quartile computation with a small window of the data. This algorithm works well for this exercise since the data wouldn't ever be anomalous in my environment.  However, the Agile or Robust algorithms might be preferable in production as they consider level shifts, long term anomalies, and seasonal trends in the data.
...
To configure a monitor, I navigated to Monitors > New Monitors and selected the option for a Metrics Monitor.
...
For conditional notifications based upon the value of `my_metric` over the 5-minute window, I followed the [Notifications](https://docs.datadoghq.com/monitors/notifications) docs and produced the following message with conditional formatting:
...
The monitor produces the following three conditional alert states:
...
I navigated to Monitors > Manage Downtime in the UI and selected the option to "Schedule Downtime."  I then configured the two windows of downtime with the following:
...
Lastly, here are some screenshots of the alerts these downtimes produced:
...
Since this provides only the basics for APM instrumentation, I opted to dig into some of the suggested further reading for [Python Runtime Metrics](https://docs.datadoghq.com/tracing/runtime_metrics/python/) and modified my `ddtrace-run` command to include the following:
...
After starting my application and validating that I was indeed getting metrics like `runtime.python.cpu.percent`, I continued to dive deeper into the possibilities of instrumentation.  This led me to an article covering [Python Custom Instrumentation](https://docs.datadoghq.com/tracing/setup_overview/custom_instrumentation/python/?tab=decorator) which enabled me to define traces for specific endpoints of my flask app like `/api/apm` and `/api/trace`.  I did so by declaring the following in [`/code/flask-app.py`](/code/flask-app.py):

    @app.route('/api/apm')
    # tracer added to track the APM endpoint
    @tracer.wrap('flask-request', service='flask-app', resource='APM', span_type='web')
    def apm_endpoint():
        return 'Getting APM Started'

    @app.route('/api/trace')
    # tracer added to track the TRACE endpoint
    @tracer.wrap('flask-request', service='flask-app', resource='TRACE', span_type='web')
    def trace_endpoint():
        return 'Posting Traces'


Now that I have instrumented my application with traces, I created a [dashboard](https://p.datadoghq.com/sb/7my7we8zuethfnx9-c3c64a55e5c789c6542a6fa97fdc34c8) of APM and infrastructure metrics:

![APM/System Dashboard](screenshots/apm_dash.PNG "APM/System Dashboard")

### **Bonus Question: What is the difference between a Service and a Resource?**

A service can be thought of as a collection of resources.  For example, the flask application built for this exercise would be considered a service, while the individual endpoints in that application like `/api/apm` and `/api/trace` are resources.
...
As an SE at Tableau, I wrote a custom Node.js application which embedded an SE time-tracking dashboard and enabled the end user to quickly and consistently log their customer facing time with meeting details, notes, stakeholders, etc.  This application quickly grew in popularity amongst both SEs and SE leadership and is currently deployed in production to over 800 people globally. 

 The application is architected around three different APIs, its own custom internal API service, and a data visualization element.  Up until now, the focus of this tool was on MVP functionality.  However, the natural next step is to address operational needs as it relates to monitoring and maintainability.
 
 Currently, I am only made aware of downtime or unexpected behavior from interacting with end users over Slack.  With this level of reporting, I only get as much information about what is wrong as the end user provides (oftentimes getting little-to-no system-level details). Datadog's platform would help me monitor the system status and enable me to rapidly diagnose application issues. Finally, I could provide real time updates on service availability to my end users to reduce the amount of administrative overhead and improve overall end-user quality of service.