...
I was unable to get the provided Flask app to run (I believe due to compatibility issues with my Win10 OS).  I instead modified the code to a native python application that simply prints a string.  

Once the program was could run my machine all I had to do was use the tracer.wrap function to tag the relevant operation name, service, and resource.  Being an increadibly simple application, the trace's recoded processing-time was near 0.
...
For example the IT group might see the project managers barely tax their machines and then start sourcing lower powered devices to shave costs.  Conversely the creative teams may be constantly overclocking their machines so IT could source upgrades to reduce render times and improve effeciency.  For purely selfish reasons this use case may help me make the argument to upgrade my wildly under-spec'd PC.  You could track anomalies in performance that may relate to misusage/malware or other generic IT concerns.
