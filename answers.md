...
The link to my Timeboard is https://app.datadoghq.com/dashboard/2up-6yx-uk5/jacky-timeboard-for-exercise. I was able to create the Timeboard by executing the following python script.
``` timeboard.py
...
Here's the configuration snapshot to achieve the requirements.
...
![Screenshot of warn](screenshots/3_warning.png)
...
* **Bonus Question**: What is the difference between a Service and a Resource?
    * A Resource is a particular action for a service. In this Flask example, the endpoints (/, /api/apm, /api/trace) are the resources to compose the Jacky Flask Service. 
...
* **Air condition notifier** : We have concerned about micro dust problem recently. When the air condition is bad due to the micro dust, we are advised to reduce outdoor activities or to wear mask for health. It can be a good idea to leaverage Datadog to notify people of air condition change.
    * data source: There are open API services to provide us with the information of air condition. We can gather micro dust level periodically from the open API services and push the data to Datadog using Datadog agent.
    * data processing: Leverage Datadog Monitor for setting notification for alert/warn/status change and no data to those who might want to get the information.
    * notification: We can get the inforamtion via email or slack immediately. This will greatly help us get update to date without checking weather news periodically.
