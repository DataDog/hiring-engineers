...
* Visualizing Data
  * Once we have collected Metrics, how can we use Datadog to visualize them?
* Monitoring Data
  * After the data have been visualized, how do you set up Monitors so that someone does not need to be watching the dashboard indefinitely for anomalies?
* Collecting APM Data
  * Now that we are collecting, visualizing, and monitoring our data and metrics, what else is there? Application Performance Monitoring lets you deep dive into your application's performance.
...
* Here we have authorized Datadog to access all of the metrics that AWS already captures for our use in Datadog
...
#### Create the custom Agent check configuration file.
``` 
...
```
  ...
  * Create the python method that contains the functionality of the custom check.
``` 
...
### How do we keep all of the data coming into Datadog from many different host organized?
...
* SUBTASK: Visualize any metric from the Integration on your Database with the anomaly function applied.
  * You can apply anomaly detection to different metrics, that will help you notice that something is out of the ordinary. Here we see the peaks are drawn in <span style="color:red">RED</span> because they are anomalous to the previous data.
  * <img src="images/dynamodb_check_failed_anomaly.png" >
  * Here is an example the anomaly function applied to my_metric, this is a little easier to see what is happening here. The graph has the grayed area indicating the expected range of the metrics value, and highlights in RED when the metric breaks that range.
...
* SUBTASK: Visualize your custom metric with the rollup function applied to sum up all the points for the past hour into a rolling summation.
  * You can create custom functions to help provide the easiest visuals to analyze. Here we are creating a "rollup" of the summation of one hours worth of my_metric, this may be helpful when you are not worried about the minute to minute changes that a gauge datatype can return, but the larger picture of the trend of that gauge.
...
Being able to configure the monitor notification messages is a power tool that ensures that when emails are going out, they are only going to ge specific group needed, and they will contain all of the important information for the task to be addressed immediately.
...
Weather it is a reoccurring maintenance or low priority monitor that is not important after dark, you can schedule downtime for a monitor which will mute if from sending out notifications during the specified timeframe.
...
After these commands have run the system will make the services tab under APM available.
Here is what the APM Services data will look like:
<img src="images/apm_dashboard.png" >
<img src="images/apm_endpoints.png" >


We can see all of the APM data coming into the APM dashboard but how can we add these graphs one of our pre-built dashboards?
...
* Monitoring and Visualizing Blood Sugar levels for people with diabetes. The healthcare industry is trying to keep up quickly changing technology, but often fall well behind of the curve. I have a few friends who have Diabetes and their monitoring solutions all seem archaic. Luckily there are now technologies that can continuously monitor ones blood sugar levels and update some cloud application via WiFi. If that infrastructure is there, we can integrate those web applications with Datadog. This could give an individual real power over monitoring their blood sugar levels with powerful tools like anomaly detection, and Monitor notifications to keep a friend or family member in the loop. I would be curious to see a metric gauge on someone's blood sugar, and then apply the Anomalies function like we did in this exercise. Could we get the parameters into a place that the anomalies would be correctly reported around insulin spikes when the individual injects, blood sugar spikes when somebody eats, if the individual keeps a stong habit, the anomaly detection may be able to find a useful pattern.
