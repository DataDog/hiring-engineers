...
The trial version expired when I was doing Level 5 so I had to switch. I also switched the OS from Windows to Ubuntu to make the agentcheck installation easier. Indeed,  I have tried the project in both windows and unix, and found out that it's easier to do it in unix ( Ubuntu 12) because the tutorial is more adapted to Unix, especially for the location of the different check and configuration folders during the level5 for the agentcheck. Furthermore, the troubleshooting in windows was painful for the Level 5, so I decided to do the level5 in Ubuntu only
