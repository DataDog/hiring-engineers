...
I would personally use it for my own project, [a stock news aggregator](https://www.pulsr.se), where I would feed Datadog with relevant data (such as tags being: ticker and other relevant information that applies to a stock), but also metrics that are of value (such as stock quotes). This does not only give me a visual and granular representation of my data, but I can also benefit from the metric alerting and webhook features that can forward the results of interest and value back to my application.
