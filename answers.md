...
### Visualizing Data  
Not having coded structured json before (in a prior role I sometimes had to review untructured json generated by daily logging outpt of software product) I chose to work "back into" generating creating a Timeboard via structred json pushed up to the Datadog SAAS via the API.  

I first created a working example Timeboard, via the GUI, and used the API to issue a query show a list of all my timeboards, including their ID number.  And then used the *timeboard_id* to downloaded the json definition of the timeboard.  I utilized *dogshell* to issue API queries these API queries.  In this endeavor *dogshell* was my friend.  
