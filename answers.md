...
* My API key and app key are encrypted using Fernet key encryption using my [script](https://github.com/kmglassman/hiring-engineers/blob/kmglassman-answers-test/scripts/enc_key.py) `enc_key.py`'.
...
These configurations allow my email notifications to be customized depending on the condition of the monitor trigger - Warning, Alert, or No Data.  Here is an example of the Alert notification, as received by email:
...
I now am gathering meaningful analytics on my application in the Datadog APM solution.  Here are some APM and infrastructure metrics I gathered and compiled in a dashboard:
